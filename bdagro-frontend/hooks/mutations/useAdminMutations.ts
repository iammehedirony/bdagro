...
export function useRejectAdminLoanApplicationMutation() { const api = useApi(); const queryClient = useQueryClient(); return useMutation({ mutationFn: ({ id, rejectionReason }: { id: string; rejectionReason: string }) => rejectAdminLoanApplication(api, id, rejectionReason), onSuccess: () => invalidateAdminQueries(queryClient) }); }