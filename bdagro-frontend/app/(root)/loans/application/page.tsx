    ...
    <div className="min-h-screen bg-white">
      <div className="mx-auto max-w-6xl px-6 py-10">
        ...
        <div className="mt-8 grid gap-10 md:grid-cols-[220px_1fr] lg:grid-cols-[220px_1fr_340px]">
          ...
          <div className="h-fit lg:sticky lg:top-6"><div className="border border-stone-200"><div className="flex items-start gap-4 border-b border-stone-200 p-5"><div className="flex h-11 w-11 shrink-0 items-center justify-center bg-emerald-900"><Wheat className="h-5 w-5 text-white/80" /></div><div><div className="text-xs text-stone-400">ফান্ডিং প্রোডাক্ট</div><div className="text-stone-900">{selectedProduct?.name ?? "প্রোডাক্ট নির্বাচন করুন"}</div></div></div><div className="space-y-4 p-5"><div className="flex items-center justify-between text-sm"><span className="flex items-center gap-1.5 text-stone-400"><HandCoins className="h-3.5 w-3.5" />মুনাফা হার</span><span className="text-stone-800">{selectedProduct ? `${selectedProduct.profitSharePercent}%` : "-"}</span></div><div className="flex items-center justify-between text-sm"><span className="flex items-center gap-1.5 text-stone-400"><Banknote className="h-3.5 w-3.5" />আবেদনকৃত পরিমাণ</span><span className="text-stone-800">{formatCurrency(Number(amount) || 0)}</span></div><div className="flex items-center justify-between text-sm"><span className="flex items-center gap-1.5 text-stone-400"><CalendarClock className="h-3.5 w-3.5" />মেয়াদ</span><span className="text-stone-800">{duration || 0} মাস</span></div><div className="border-t border-stone-200 pt-4 text-xs leading-relaxed text-stone-500">{selectedProduct?.description ?? "প্রোডাক্ট নির্বাচন করলে তার শর্তাবলী এখানে দেখা যাবে।"}</div></div></div></div>
        </div>
      </div>
    </div>
  );
