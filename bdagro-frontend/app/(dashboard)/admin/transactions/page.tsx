      ...
      <div className="p-4 lg:p-0">
        ...
        <div className="border border-stone-200">
          ...
          <div className="w-full overflow-x-auto">
            <div className="divide-y divide-stone-200 min-w-[700px]">
              ...
              {!isLoading && !isError && transactions.map((transaction) => {
                ...
                return <div key={transaction._id} className="p-4 lg:p-5 flex items-center gap-3 lg:gap-4 flex-wrap">
                  <div className="w-9 h-9 flex items-center justify-center bg-stone-100 text-stone-500 shrink-0">
                    <Icon className="w-4 h-4" />
                  </div>
                  <div className="flex-1 min-w-0">
                    <div className="text-sm text-stone-800 break-words whitespace-normal">{transactionTitle(transaction)}</div>
                    <div className="text-xs text-stone-400 mt-0.5 truncate">{transaction.project?.title ?? "অজানা প্রকল্প"} · {formatDate(transaction.createdAt)}</div>
                  </div>
