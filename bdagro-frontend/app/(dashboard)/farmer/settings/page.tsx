...
export default function FarmerSettingsPage() {
  ...
  const { mutate: updateProfile, isPending: isUpdatingProfile } = useUpdateFarmerProfileMutation();
  ...
  const getVerificationStatus = () => {
    if (!profile) return { status: "pending" as const, label: "অপেক্ষমাণ", color: "amber" };
    switch (profile.verificationStatus) {
      case "approved":
        return { status: "approved" as const, label: "যাচাইকৃত", color: "emerald" };
      case "rejected":
        return { status: "rejected" as const, label: "বাতিল", color: "red" };
      case "pending":
      default:
        return { status: "pending" as const, label: "অপেক্ষমাণ", color: "amber" };
    }
  };

  const verification = getVerificationStatus();
  const verificationColors = {
    approved: { bg: "bg-emerald-50", border: "border-emerald-600", text: "text-emerald-700", badge: "bg-emerald-50 border-emerald-600 text-emerald-800" },
    rejected: { bg: "bg-red-50", border: "border-red-600", text: "text-red-700", badge: "bg-red-50 border-red-600 text-red-800" },
    pending: { bg: "bg-amber-50", border: "border-amber-600", text: "text-amber-700", badge: "bg-amber-50 border-amber-600 text-amber-800" },
  };
      ...
      <div className="flex-1 min-w-0">
        <div className="p-8 max-w-2xl space-y-10">
          ...
          <div>
            ...
            <form onSubmit={handleProfileSubmit(onProfileSubmit)} className="space-y-4">
              <div className="border border-stone-200 p-6 grid sm:grid-cols-2 gap-5">
                ...
                <div>
                  <Field label="ঠিকানা" {...registerProfile("address")} placeholder="জেলা, উপজেলা, গ্রাম" />
              ...
              <div className="flex items-center gap-3">
                <button
                  ...
                  className="bg-emerald-900 text-white px-6 py-3 text-sm hover:bg-emerald-800 disabled:opacity-50 flex items-center"
                >
          ...
          <div>
            ...
            <div className={`border ${vColors.border} p-6 flex items-center gap-4`}>
              <div className={`w-11 h-11 ${vColors.bg} ${vColors.border} rounded-full flex items-center justify-center shrink-0`}>
                <ShieldCheck className={`w-5 h-5 ${vColors.text}`} />
              </div>
              <div className="flex-1">
                <div className="text-sm text-stone-800">{verification.label}</div>
                <div className="text-xs text-stone-400 mt-0.5">
                  NID নম্বর: {profile ? maskNid(profile.nidNumber) : "—"} · ভেরিফাইড {formatDate(profile?.verifiedAt)}
                </div>
              </div>
              <span className={`text-xs ${vColors.badge} px-2 py-0.5`}>
                {verification.label}
              </span>
            </div>
          </div>
            ...
            <form onSubmit={handlePasswordSubmit(onPasswordSubmit)} className="space-y-4">
              ...
              <div className="flex items-center gap-3">
                ...
                <button
                  type="button"
                  onClick={() => signOut()}
                  className="text-sm text-stone-500 hover:text-stone-800 flex items-center gap-1.5 ml-auto"
                >
